...
const Projects = () => {
  ...
  return (
    <section id="projects" className="section bg-black px-6 py-24 flex justify-center fade-in-up">
      <div className="max-w-[980px] w-full mx-auto">
        <h2 className="text-[clamp(2.5rem,5vw,4rem)] font-extrabold tracking-[-0.04em] mb-16 text-[#f5f5f7] leading-none text-center">
          Featured Projects.
        </h2>
        ...
        <div className="flex flex-col gap-16 sm:gap-20">
          {projects.map((proj, idx) => (
            <div key={idx} className={`flex flex-col ${idx % 2 === 1 ? 'md:flex-row-reverse' : 'md:flex-row'} gap-8 md:gap-12 items-center group`}>
              ...
              {/* Image Section */}
              <div className="w-full md:w-[45%] aspect-[4/3] rounded-[2rem] bg-[#050505] overflow-hidden relative border border-white/[0.08] shadow-2xl group-hover:border-white/[0.15] transition-all duration-500">
                ...
                {/* Floating Date Badge */}
                <h4 className="absolute bottom-5 left-5 text-[10px] text-white font-bold tracking-[0.2em] uppercase bg-white/[0.05] backdrop-blur-xl px-4 py-2 rounded-full border border-white/10 shadow-lg">
                  {proj.date}
                </h4>
              </div>
              ...
              <div className="w-full md:w-[55%] flex flex-col justify-center">
                <h3 className="text-2xl sm:text-3xl font-bold text-[#f5f5f7] mb-4 leading-tight tracking-[-0.02em] group-hover:text-white transition-colors">{proj.title}</h3>
                
                {/* Tech Stack Action Pills */}
                <div className="flex flex-wrap gap-2 mb-6">
                  {proj.tech.map((t, i) => (
                    <div key={i} className="flex items-center gap-1.5 bg-[#050505] border border-white/[0.08] px-3 py-1.5 rounded-full backdrop-blur-sm group/tech transition-all duration-300 shadow-sm hover:border-white/[0.2] hover:bg-white/[0.05]">
                      ...
                      <span className="text-[10px] text-[#86868b] font-bold tracking-[0.1em] group-hover/tech:text-white transition-colors uppercase">{t.name}</span>
                    </div>
                  ))}
                </div>
                ...
                <ul className="space-y-3 mb-8">
                  {proj.bullets.map((bullet, i) => (
                    <li key={i} className="flex items-start text-[14px] sm:text-[15px] text-[#a1a1a6] font-medium leading-[1.7]">
                ...
                <div className="flex items-center gap-3 border-t border-white/[0.08] pt-6">
                   {proj.live !== "#" ? (
                      <a href={proj.live} target="_blank" rel="noopener noreferrer" className="flex justify-center items-center text-xs font-bold text-black bg-white hover:bg-gray-200 px-6 py-3 rounded-full transition-transform hover:scale-[1.03] active:scale-95 shadow-md">
                        Live Demo <span className="ml-1 text-[10px]">↗</span>
                      </a>
                   ) : (
                      <span className="flex justify-center items-center text-[10px] uppercase tracking-widest text-[#86868b] font-bold bg-[#050505] border border-white/[0.08] px-5 py-3 rounded-full opacity-60">
                        Console App
                      </span>
                   )}
                   <a href={proj.link} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center text-xs font-bold text-[#f5f5f7] bg-[#050505] hover:bg-white/[0.08] px-6 py-3 rounded-full transition-transform hover:scale-[1.03] active:scale-95 border border-white/[0.15] shadow-sm">
                     View Source
